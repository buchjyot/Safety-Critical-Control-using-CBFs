...
%% Design CLF-CBF
odeParams.beta = 0.5;
[tCLFCBF1,xCLFCBF1] = ode45(@(t,x) hCLFCBFDyn(t,x,odeParams),[0 500],x0,odeOpt);

odeParams.beta = 1;
[tCLFCBF2,xCLFCBF2] = ode45(@(t,x) hCLFCBFDyn(t,x,odeParams),[0 500],x0,odeOpt);

odeParams.beta = 2;
[tCLFCBF3,xCLFCBF3] = ode45(@(t,x) hCLFCBFDyn(t,x,odeParams),[0 500],x0,odeOpt);
...
p4 = plot(xCLFCBF1(:,1),xCLFCBF1(:,2),'-.c','LineWidth',2);
p5 = plot(xCLFCBF2(:,1),xCLFCBF2(:,2),'--m','LineWidth',2);
p6 = plot(xCLFCBF3(:,1),xCLFCBF3(:,2),'g','LineWidth',2);
...
xlim([-20 20]);
ylim([-15 15]);
legend([p1 p2 p3 p4 p5 p6],'Openloop','LQR Baseline','Unsafe Region','CLF-CBF QP \gamma = 0.5','CLF-CBF QP \gamma = 1','CLF-CBF QP \gamma = 2','FontSize',12,'Location','northwest');